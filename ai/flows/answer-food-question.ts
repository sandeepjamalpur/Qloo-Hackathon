...
export async function answerFoodQuestion(input: AnswerFoodQuestionInput): Promise<AnswerFoodQuestionOutput> {
  try {
    ...
    return await queryHuggingFaceQA(input.question, input.context);
  } catch (error) {
    console.error('Error in answerFoodQuestion flow:', error);
    return "I'm sorry, but I encountered an error while trying to find an answer. Please try again later.";
  }
}
