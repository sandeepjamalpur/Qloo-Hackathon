  ...
  async (query) => {
    try {
      const { media } = await ai.generate({
        // Use a more stable image generation model
        model: 'googleai/gemini-1.5-flash-exp',
        prompt: `A beautiful, high-quality, artistic representation of the following Indian cultural item: ${query}. The style should be visually stunning, evocative, and photorealistic.`,
        config: {
