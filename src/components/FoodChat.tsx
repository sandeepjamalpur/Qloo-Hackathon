...
export function FoodChat({ placeholder, title, description }: FoodChatProps) {
    ...
    const handleChatSubmit = async () => {
        ...
        try {
            const answer = await answerFoodQuestion({ question: currentInput, context: foodContext, chatHistory: newHistory });
    ...
    return (
        <div>
            {title && <h3 className="font-headline text-xl mb-1">{title}</h3>}
