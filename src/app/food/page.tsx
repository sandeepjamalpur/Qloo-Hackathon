...
import { ChefHat, Loader2, Heart, MapPin, ImageOff, Search, Mic, BookOpen, Soup, Flame, Zap, Wheat, Droplets, Package } from 'lucide-react';
...
const FeaturedRecipeCard = ({ item, onRecipeClick, onDetailClick }: { item: CulturalItem, onRecipeClick: () => void, onDetailClick: () => void }) => (
    <div className="bg-accent text-accent-foreground p-8 md:p-10 rounded-lg shadow-lg max-w-md w-full">
      ...
      {item.calories && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-center border-t border-accent-foreground/20 pt-4 mb-6">
          <div className="flex flex-col items-center">
            <Package className="w-5 h-5 mb-1" />
            <p className="font-bold text-lg">{item.quantity || '-'}</p>
            <p className="text-xs opacity-80">Quantity</p>
          </div>
          <div className="flex flex-col items-center">
          ...
          <div className="flex flex-col items-center">
            ...
            <p className="font-bold text-lg">{item.protein}</p>
            <p className="text-xs opacity-80">Protein</p>
          </div>
          <div className="flex flex-col items-center">
            <Wheat className="w-5 h-5 mb-1" />
            <p className="font-bold text-lg">{item.carbs}</p>
            <p className="text-xs opacity-80">Carbs</p>
          </div>
          <div className="flex flex-col items-center">
            <Droplets className="w-5 h-5 mb-1" />
            <p className="font-bold text-lg">{item.fat}</p>
      ...
      <div className="flex gap-2">
        ...
        <Button variant="outline" onClick={onRecipeClick} className="bg-accent-foreground/10 hover:bg-accent-foreground/20 text-accent-foreground border-accent-foreground/20">
    ...
    return (
        <>
            <div className={cn(
                ...
                <FeaturedRecipeCard 
                    item={item} 
                    onRecipeClick={() => setIsRecipeOpen(true)}
