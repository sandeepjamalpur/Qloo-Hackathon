...
1.  **Filter-Based Search**:
    *   If a 'place' (a specific Indian state) is provided, you MUST categorize all returned items as 'Place'. This is the most critical rule. The user is on a "Places" page, so everything you return must be a place.
    ...
    *   You MUST generate a list containing a MINIMUM of 5 items. Aim for 5 to 8 items.
...
3.  **Gather Details**: For each item in your generated list, provide the following in the structured output format:
    ...
    *   **calories, protein, carbs, fat, quantity**: If the category is 'Food', provide estimated nutritional information, including a typical serving quantity.
`,
});
